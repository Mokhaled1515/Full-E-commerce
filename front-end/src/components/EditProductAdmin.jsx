...
const EditProductAdmin = ({close,data: propsData, fetchProductData}) => {
  ...
  return (
    <section className="fixed top-0 right-0 left-0 bottom-0 p-4 bg-black/70 z-50">
      <div className="dark:bg-black bg-white w-full p-4 max-w-2xl scrollcarCustome mx-auto rounded overflow-y-auto h-full max-h-[95vh]">

        <section className="">
          <div className="p-2 shadow-md flex justify-between items-center">
            <h2 className="font-semibold ">Upload Product</h2>
            <button className="cursor-pointer text-red-500 hover:rotate-180 hover:transition-all hover:duration-150" onClick={close}>
                <IoClose size={20}/>
            </button>
          </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {allCategory.map((c, index) => {
                      return <option key={c+"SelectCategory"+index} value={c?._id}>{c.name}</option>;
                    })}
                  </select>
         ...
         py-2 rounded font-semibold my-4 cursor-pointer dark:hover:bg-gray-600 dark:text-white dark:bg-gray-500"
              >
                Update Product
              </button>
            </form>
