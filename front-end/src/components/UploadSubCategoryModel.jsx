...
const UploadSubCategoryModel = ({ close, fetchData }) => {
  ...
  // }
  const handleRemoveCategorySelected = (categoryId) => {
    if (!subcategoryData || !Array.isArray(subcategoryData.category)) {
      console.error("subcategoryData أو category غير معرّفين");
      return;
    }
    ...
    if (index > -1) {
      ...
      setsubcategoryData((prev) => ({
        ...prev,
        category: [...subcategoryData.category], // لضمان إعادة التصيير (re-render)
      }));
    }
  };
          ...
          <div className="grid gap-1">
            ...
            >
                  ...
                  subcategoryData.category?.length > 0 &&
                    subcategoryData.category.map((cat, index) => (
                      <p
                        ...
                        className="bg-white dark:bg-black shadow-md px-1 m-1 flex items-center gap-2"
                      >
                        {cat.name}
                        <div
              ...
              <select
                name=""
                id=""
                className="w-full p-2 bg-transparent dark:bg-black cursor-pointer outline-none "
                onChange={(e) => {
