...
const OtpVerification = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await Axios({
        ...SummaryApi.forgot_password_verify,
        data: {
          otp: data.join(""),
  ...
  return (
    <section className="w-full flex container mx-auto px-2">
              ...
              {data.map((element, index) => {
                return (
                  <input
                    ...
                    maxLength={1}
                    className="bg-blue-50 dark:bg-neutral-900 w-full dark:text-orange-500 max-w-16 p-2 border rounded outline-none focus:border-primary-200 text-center font-semibold"
                  />
                );
              })}
            </div>
